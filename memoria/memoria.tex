...
\section{Motivation}
If we make an analysis of current society it's easy to observe that the use of mobile devices is a constant in the day to day of each person. And that the functionality they offer increases at a great speed. These devices are already an essential part of our lives, and as consequence their applications too.

One of the strengths of these devices it's the help they give us to process all the information on the network. Applications with all that information help us broaden our perception of the world and to make decisions. Another important asset of the new technologies are the social character that they are acquiring, with recommendations, photos, likes ... Within these social applications the most used are instant messaging that put us in touch with our friends and acquaintances, and allow us to share all kind of information.

In some of the conversations with our friends there were complaints of not having a simple and friendly tool to find bars, restaurants and other types of nearby leisure facilities. The first thing you think to look for a solution to this problem is in the \texttt{Google Maps} application. But this app it isn't comfortable but it's embedded in another applications due to the huge amount of information that it offers.

In this situation, we thought that a \texttt{Telegram} bot would fit well as a solution. On the one hand, \texttt{Telegram} is an application that we use every day and a bot would be easy to handle. On the other hand, its social character allows us to have feedback with the users that allows us to establish which are the best options that \texttt{Google Maps} offers us.
...
\lstinputlisting[language=Python, firstline=323, lastline=330, title=Ejemplo de creación de un \textit{DelegatorBot}]{../socialBot.py}
...
Al comenzar a codificar el bot, creé la estructura base del bot a partir de los bots que había desarrollado en la fase de experimentación. Me encargué también de preparar la \textit{API} de \texttt{Google Maps} para poder usarla, registrándose y obteniendo la clave. Hice las primeras pruebas del método \textit{places\_nearby}, aunque en este momento del desarrollo aún no nos dimos cuenta de que este método no funcionaba correctamente. 
